...
function w3tc_minify_js_file_add(theme, template, location, file) {
  var append = jQuery(
    '<li><table><tr><th>&nbsp;</th><th>File URI:</th><th>Template:</th><th colspan="3">Embed Location:</th></tr><tr><td>' +
      (jQuery("#js_files li").length + 1) +
      '.</td><td><input class="js_enabled" type="text" name="js_files[' +
      theme +
      "][" +
      template +
      "][" +
      location +
      '][]" value="" size="70" \/></td><td><select class="js_file_template js_enabled"></select></td><td><select class="js_file_location js_enabled"><option value="include">Embed in &lt;head&gt;</option><option value="include-body">Embed after &lt;body&gt;</option><option value="include-footer">Embed before &lt;/body&gt;</option></select></td><td><input class="js_file_delete js_enabled button" type="button" value="Delete" /> <input class="js_file_verify js_enabled button" type="button" value="Verify URI" /></td></tr></table><\/li>',
  );
...
function w3tc_minify_css_file_add(theme, template, file) {
  var append = jQuery(
    '<li><table><tr><th>&nbsp;</th><th>File URI:</th><th colspan="2">Template:</th></tr><tr><td>' +
      (jQuery("#css_files li").length + 1) +
      '.</td><td><input class="css_enabled" type="text" name="css_files[' +
      theme +
      "][" +
      template +
      '][include][]" value="" size="70" \/></td><td><select class="css_file_template css_enabled"></select></td><td><input class="css_file_delete css_enabled button" type="button" value="Delete" /></td><td><input class="css_file_verify css_enabled button" type="button" value="Verify URI" /></td></tr></table><\/li>',
  );
  append.find("input:text").val(file);
...
function w3tc_toggle2(name, dependent_ids) {
  ...
  jQuery(dependants).on("click", function () {
    ...
    jQuery(dependants).each(function () {
      ...
      if (!current_checked) total_checked = false;
    });
...
function w3tc_security_headers() {
  var directive_description = {
    browsercache_security_hsts_directive: {
      maxage:
        'The time, in seconds (as defined under the "Expires Header Lifetime" box of "Media & Other Files"), that the browser should remember that this site is only to be accessed using <acronym title="HyperText Transfer Protocol over SSL">HTTPS</acronym>. This only affects the site\'s main domain.',
      maxagepre:
      ...
      maxageinc:
        'The time, in seconds (as defined under the "Expires Header Lifetime" box of "Media & Other Files"), that the browser should remember that this site is only to be accessed using <acronym title="HyperText Transfer Protocol over SSL">HTTPS</acronym>. This affects the site\'s subdomains as well.',
      maxageincpre:
    ...
    browsercache_security_xss_directive: {
      ...
      block:
        "Enables <acronym title='Cross-Site Scripting'>XSS</acronym> filtering. Rather than sanitizing the page, the browser will prevent rendering of the page if an attack is detected.",
    },
    browsercache_security_pkp_extra: {
      maxage:
        'The time, in seconds (as defined under the "Expires Header Lifetime" box of "Media & Other Files"), that the browser should remember that this site is only to be accessed using one of the defined keys. This only affects the site\'s main domain.',
      maxageinc:
        'The time, in seconds (as defined under the "Expires Header Lifetime" box of "Media & Other Files"), that the browser should remember that this site is only to be accessed using one of the defined keys. This affects the site\'s subdomains as well.',
    },
  ...
  jQuery(
    "#browsercache_security_hsts_directive,#browsercache_security_xfo_directive,#browsercache_security_xss_directive,#browsercache_security_pkp_extra,#browsercache_security_pkp_report_only",
  ).on("change", function () {
    jQuery("#" + jQuery(this).attr("id") + "_description").html(
      "<i>" +
        directive_description[jQuery(this).attr("id")][jQuery(this).val()] +
        "</i>",
    );
...
function w3tc_csp_reference() {
  W3tc_Lightbox.open({
    ...
    url:
      ...
      w3tcGetAjaxNonce("browsercache_quick_reference") +
...
function cdn_cf_check() {
  ...
  // General page.
  if (
    !w3tcData.cdnFlushManually &&
    cdnEnabled &&
    ("cf" === cdnEngine || "cf2" === cdnEngine)
  ) {
  ...
  if (
    !cdnFlushManually &&
    w3tcData.cdnEnabled &&
    ("cf" === w3tcData.cdnEngine || "cf2" === w3tcData.cdnEngine)
  ) {
...
function set_sticky_bar_positions() {
  jQuery("#w3tc > #w3tc-options-menu").css(
    "top",
    Math.floor(
      (jQuery(window).width() > 600 && jQuery("#wpadminbar").length
        ? jQuery("#wpadminbar").outerHeight()
        : 0) + jQuery("#w3tc-top-nav-bar").outerHeight(),
    ),
  );
  jQuery(".w3tc_form_bar").css(
    ...
    Math.floor(
      (jQuery(window).width() > 600 && jQuery("#wpadminbar").length
        ...
        (jQuery("#w3tc > #w3tc-options-menu").length
          ? jQuery("#w3tc > #w3tc-options-menu").outerHeight()
          : 0),
    ),
  );
}
...
function cdn_cf_bucket_location() {
  ...
  jQuery("#cdn-cf-bucket-hostname").text(
    id +
      ".s3." +
      get_bucket_region(jQuery("#cdn_cf_bucket_location").val()) +
      "amazonaws.com",
  );
}
...
function toggle_dbcache_notice() {
  jQuery(".dbcache_disk_notice").toggle(
    jQuery("#dbcache__engine").val() === "file" &&
      jQuery("#dbcache__enabled").is(":checked"),
  );
}
...
function toggle_objectcache_notice() {
  jQuery(".objectcache_disk_notice").toggle(
    jQuery("#objectcache__engine").val() === "file" &&
      jQuery("#objectcache__enabled").is(":checked"),
  );
}
...
function toggle_fragmentcache_notice() {
  jQuery(".fragmentcache_disk_notice").toggle(
    jQuery("#fragmentcache___engine").val() === "file",
  );
}
...
jQuery(function () {
  ...
  jQuery(document).on(
    "click",
    ".performance_page_w3tc_general .nav-tab",
    function () {
  ...
  jQuery(document).on("click", '[data-tab-type="help"]', function () {
    ...
    jQuery.ajax({
      ...
      data: {
        ...
        _wpnonce: w3tc_forums_api_nonce[0],
        tabId: tabId,
      },
  ...
  // Pagecache page.
  w3tc_input_enable(
    "#pgcache_reject_roles input[type=checkbox]",
    jQuery("#pgcache__reject__logged_roles:checked").length,
  );
  ...
  w3tc_toggle2("browsercache_last_modified", [
    "browsercache__cssjs__last_modified",
    "browsercache__html__last_modified",
    "browsercache__other__last_modified",
  ]);
  w3tc_toggle2("browsercache_expires", [
    "browsercache__cssjs__expires",
    "browsercache__html__expires",
    "browsercache__other__expires",
  ]);
  w3tc_toggle2("browsercache_cache_control", [
    "browsercache__cssjs__cache__control",
    "browsercache__html__cache__control",
    "browsercache__other__cache__control",
  ]);
  w3tc_toggle2("browsercache_etag", [
    "browsercache__cssjs__etag",
    "browsercache__html__etag",
    "browsercache__other__etag",
  ]);
  w3tc_toggle2("browsercache_w3tc", [
    "browsercache__cssjs__w3tc",
    "browsercache__html__w3tc",
    "browsercache__other__w3tc",
  ]);
  w3tc_toggle2("browsercache_compression", [
    "browsercache__cssjs__compression",
    "browsercache__html__compression",
    "browsercache__other__compression",
  ]);
  w3tc_toggle2("browsercache_brotli", [
    "browsercache__cssjs__brotli",
    "browsercache__html__brotli",
    "browsercache__other__brotli",
  ]);
  w3tc_toggle2("browsercache_replace", [
    "browsercache__cssjs__replace",
    "browsercache__other__replace",
  ]);
  w3tc_toggle2("browsercache_querystring", [
    "browsercache__cssjs__querystring",
    "browsercache__other__querystring",
  ]);
  w3tc_toggle2("browsercache_nocookies", [
    "browsercache__cssjs__nocookies",
    "browsercache__other__nocookies",
  ]);
  ...
  w3tc_input_enable(
    ".html_enabled",
    jQuery("#minify__html__enable:checked").length,
  );
  w3tc_input_enable(
    ".js_enabled",
    jQuery("#minify__js__enable:checked").length,
  );
  w3tc_input_enable(
    ".css_enabled",
    jQuery("#minify__css__enable:checked").length,
  );
  ...
  jQuery(".js_file_template").on("change", function () {
    jQuery(this)
      ...
      .attr(
        "name",
        "js_files[" +
          jQuery("#js_themes").val() +
          "][" +
          jQuery(this).val() +
          "][" +
          jQuery(this).parents("li").find(".js_file_location").val() +
          "][]",
      );
  });
    ...
    jQuery(this)
      ...
      .attr(
        ...
        "js_files[" +
          jQuery("#js_themes").val() +
          "][" +
          jQuery(this).parents("li").find(".js_file_template").val() +
          "][" +
          jQuery(this).val() +
          "][]",
      );
  ...
  jQuery(".js_file_delete").on("click", function () {
    ...
    if (
      ...
      confirm("Are you sure you want to remove this JS file?")
    ) {
      parent.remove();
      w3tc_minify_js_file_clear();
  ...
  jQuery(".css_file_delete").on("click", function () {
    ...
    if (
      parent.find("input[type=text]").val() == "" ||
      confirm("Are you sure you want to remove this CSS file?")
    ) {
      parent.remove();
      w3tc_minify_css_file_clear();
      w3tc_beforeupload_bind();
  ...
  jQuery("#minify_form").on("submit", function () {
    ...
    jQuery("#js_files :text").each(function () {
      ...
      if (match) {
        g =
          "[" +
          jQuery("#js_themes option[value=" + match[1] + "]").text() +
          "] " +
          v;
      }
      if (v != "") {
    ...
    jQuery("#css_files :text").each(function () {
      ...
      if (match) {
        g =
          "[" +
          jQuery("#css_themes option[value=" + match[1] + "]").text() +
          "] " +
          v;
      }
      if (v != "") {
    ...
    if (jQuery("#js_enabled:checked").length) {
      if (
        invalid_js.length &&
        !confirm(
          "The following files have invalid JS file extension:\r\n\r\n" +
            invalid_js.join("\r\n") +
      ...
      if (query_js.length) {
        alert(
          "We recommend using the entire URI for files with query string (GET) variables. You entered:\r\n\r\n" +
            query_js.join("\r\n"),
        );
        return false;
      }
    ...
    if (jQuery("#css_enabled:checked").length) {
      if (
        invalid_css.length &&
        !confirm(
          "The following files have invalid CSS file extension:\r\n\r\n" +
            invalid_css.join("\r\n") +
  ...
  w3tc_input_enable(
    "#cdn_reject_roles input[type=checkbox]",
    jQuery("#cdn__reject__logged_roles:checked").length,
  );
  ...
  jQuery("#cdn_export_library").on("click", function () {
    w3tc_popup(
      "admin.php?page=w3tc_cdn&w3tc_cdn_export_library&_wpnonce=" +
        jQuery(this).metadata().nonce,
      "cdn_export_library",
    );
  });
  ...
  jQuery("#cdn_import_library").on("click", function () {
    w3tc_popup(
      "admin.php?page=w3tc_cdn&w3tc_cdn_import_library&_wpnonce=" +
        jQuery(this).metadata().nonce,
      "cdn_import_library",
    );
  });
  ...
  jQuery("#cdn_queue").on("click", function () {
    w3tc_popup(
      "admin.php?page=w3tc_cdn&w3tc_cdn_queue&_wpnonce=" +
        jQuery(this).metadata().nonce,
      "cdn_queue",
    );
  });
  ...
  jQuery("#cdn_rename_domain").on("click", function () {
    w3tc_popup(
      "admin.php?page=w3tc_cdn&w3tc_cdn_rename_domain&_wpnonce=" +
        jQuery(this).metadata().nonce,
      "cdn_rename_domain",
    );
  });
  ...
  jQuery("#cdn_purge").on("click", function () {
    w3tc_popup(
      "admin.php?page=w3tc_cdn&w3tc_cdn_purge&_wpnonce=" +
        jQuery(this).metadata().nonce,
      "cdn_purge",
    );
  });
  ...
  jQuery(".cdn_export").on("click", function () {
    ...
    w3tc_popup(
      "admin.php?page=w3tc_cdn&w3tc_cdn_export&cdn_export_type=" +
        metadata.type +
        "&_wpnonce=" +
        metadata.nonce,
      "cdn_export_" + metadata.type,
    );
  });
    ...
    jQuery
      .post(
        "admin.php?page=w3tc_dashboard",
        {
          ...
          servers: jQuery("#memcached_servers")
            .val()
            .replace(/(\r\n|\r|\n)/g, ","),
          binary_protocol: jQuery("[id$=__memcached__binary_protocol]").is(
            ":checked",
          ),
  ...
  jQuery(".w3tc_common_redis_test").on("click", function () {
    ...
    jQuery
      .post(
        ...
        {
          w3tc_test_redis: 1,
          servers: jQuery("#redis_servers")
            .val()
            .replace(/(\r\n|\r|\n)/g, ","),
          verify_tls_certificates: jQuery(
            "[id$=__redis__verify_tls_certificates]",
          ).is(":checked"),
  ...
  jQuery("body").on("click", "#cdn_cname_add", function () {
    jQuery("#cdn_cnames").append(
      '<li><input type="text" name="cdn_cnames[]" value="" size="60" /> <input class="button cdn_cname_delete" type="button" value="Delete" /> <span></span></li>',
    );
  ...
  jQuery(".cdn_cname_delete").on("click", function () {
    ...
    if (
      ...
      confirm("Are you sure you want to remove this CNAME?")
    ) {
      p.remove();
      w3tc_cdn_cnames_assign();
      w3tc_beforeupload_bind();
  ...
  if (jQuery.ui && jQuery.ui.sortable) {
    ...
    jQuery("#mobile_groups").sortable({
      axis: "y",
      stop: function () {
        jQuery("#mobile_groups")
          .find(".mobile_group_number")
          .each(function (index) {
    ...
    jQuery("#referrer_groups").sortable({
      axis: "y",
      stop: function () {
        jQuery("#referrer_groups")
          .find(".referrer_group_number")
          .each(function (index) {
            jQuery(this).html(index + 1 + ".");
          });
      },
  ...
  jQuery(".w3tc-test-container-intro").click(function () {
    ...
    if ($score.css("visibility") === "hidden") {
      ...
      $score.fadeTo(300, 0, function () {
        // Fade out score element
  ...
  jQuery("#w3tc input,#w3tc select,#w3tc textarea").on("change", function () {
    ...
    jQuery(this)
      ...
      .each(function () {
        if (
          jQuery(this).hasClass("w3tc-ignore-change") ||
  ...
  // wp cron settings.
  jQuery(document).on("change", "#allcache__wp_cron", function () {
  ...
  jQuery(document).on("change", "#pgcache__wp_cron", function () {
    let $enabled = jQuery(this).prop("checked");
  ...
  jQuery(document).on("change", "#dbcache__wp_cron", function () {
    let $enabled = jQuery(this).prop("checked");
  ...
  jQuery(document).on("change", "#minify__wp_cron", function () {
    let $enabled = jQuery(this).prop("checked");
  ...
  jQuery(document).on("change", "#objectcache__wp_cron", function () {
    let $enabled = jQuery(this).prop("checked");
  ...
  function w3tc_load_faq_section(i) {
    ...
    if (w3tchelp_loaded[section]) return;

    i.html(
      '<div class="w3tchelp_loading_outer">' +
  ...
  jQuery(".w3tc_extensions_manage_input_checkall").on("click", function (v) {
    ...
    jQuery(".w3tc_extensions_input_active").each(function (index) {
      if (!jQuery(this).is(":disabled")) jQuery(this).prop("checked", c);
    });
  });
  ...
  jQuery(".w3tc-gopro-more").on("click", function (e) {
    ...
    if (!jQuery(this).data("expanded")) {
      ...
      jQuery(this).html(
        'Show Less <span class="dashicons dashicons-arrow-up-alt2"></span>',
      );
      jQuery(this)
        .parent()
        .find(".w3tc-gopro-description")
        .css("max-height", "300px");
    } else {
      jQuery(this).data("expanded", "");
      jQuery(this).html(
        'Show More <span class="dashicons dashicons-arrow-down-alt2"></span>',
      );
      jQuery(this)
        .parent()
        .find(".w3tc-gopro-description")
      ...
      var hash = this.hash,
        wpadminbar_height =
          jQuery(window).width() > 600 && jQuery("#wpadminbar").length
            ? jQuery("#wpadminbar").outerHeight()
            : 0,
        nav_bar_height = jQuery("#w3tc-top-nav-bar").length
          ? jQuery("#w3tc-top-nav-bar").outerHeight()
          : 0,
        options_menu_height = jQuery("#w3tc > #w3tc-options-menu").length
          ? jQuery("#w3tc > #w3tc-options-menu").outerHeight()
          : 0,
        form_bar_height = jQuery(".w3tc_form_bar").length
          ? jQuery(".w3tc_form_bar").outerHeight()
          : 0;
  ...
  if (hash) {
    var target_el = document.getElementById(hash.substring(1));
    if (target_el) {
  ...
  jQuery(
    ".notice:not(.inline), .updated:not(.inline), .update-nag:not(.inline), .error:not(.inline), .info:not(.inline), .warning:not(.inline)",
  ).each(function () {
