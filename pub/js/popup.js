...
var W3tc_Popup_Cdn_Export_File = {
  ...
  process_callback: function (data) {
    ...
    if (failed) {
      ...
      this.set_progress(
        ((count_processed * 100) / this.files.length).toFixed(0),
      );
...
var W3tc_Popup_Cdn_Import_Library = {
  ...
  add_rule: function (src, dst) {
    ...
    if (this.is_redirect_cdn()) {
      dst =
        "http://" +
        (this.cdn_host.length > 0 ? this.cdn_host : document.location.host) +
        "/" +
        dst;
    } else {
      dst = "/" + dst;
    ...
    rules.val(
      rules.val() +
        "Redirect " +
        (this.is_redirect_permanent() ? "302 " : "") +
        src +
        " " +
        dst +
        "\r\n",
  ...
  process: function () {
    ...
    var me = this;
    var cdn_import_external = jQuery("input#cdn_import_external").is(
      ":checked",
    );
    jQuery.ajax({
      type: "POST",
