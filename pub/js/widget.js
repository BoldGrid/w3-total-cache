jQuery(function () {
  ...
  jQuery(document).ready(function () {
    var forumLoading = jQuery("#w3tc_latest")
      .find("div.inside:visible")
      .find(".widget-loading");
    if (forumLoading.length) {
      ...
      setTimeout(function () {
        forumLoadingParent.load(
          ajaxurl +
            "?action=w3tc_widget_latest_ajax&_wpnonce=" +
            jQuery(forumLoading).metadata().nonce,
          function () {
            forumLoadingParent.hide().slideDown("normal", function () {
    ...
    var newsLoading = jQuery("#w3tc_latest_news")
      .find("div.inside:visible")
      .find(".widget-loading");
    if (newsLoading.length) {
      ...
      setTimeout(function () {
        newsLoadingParent.load(
          ajaxurl +
            "?action=w3tc_widget_latest_news_ajax&_wpnonce=" +
            jQuery(newsLoading).metadata().nonce,
          function () {
            newsLoadingParent.hide().slideDown("normal", function () {
              jQuery(this).css("display", "");
            });
          },
        );
