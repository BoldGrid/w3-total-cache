...
jQuery(function () {
  jQuery(document).on("submit", "#cachegroups_form", function () {
    ...
    mobile_groups.each(function (index, mobile_group) {
      ...
      if ($mobile_group.find(".mobile_group_enabled:checked").length) {
        ...
        mobile_groups
          .not($mobile_group)
          .each(function (index, compare_mobile_group) {
            var $compare_mobile_group = jQuery(compare_mobile_group);

            if (
              $compare_mobile_group.find(".mobile_group_enabled:checked").length
            ) {
              var compare_name = $compare_mobile_group
                .find(".mobile_group")
                .text();
              var compare_theme = $compare_mobile_group.find("select").val();
              var compare_redirect = $compare_mobile_group
                .find("input[type=text]")
                .val();
              var compare_agents = $compare_mobile_group
                .find("textarea")
                .val()
                .split("\n")
                .filter(function (line) {
              ...
              if (
                ...
                error.push(
                  'Duplicate theme "' +
                    compare_theme +
                    '" found in the user agent groups "' +
                    groups[0] +
                    '" and "' +
                    groups[1] +
                    '"',
    ...
    referrer_groups.each(function (index, referrer_group) {
      ...
      if ($referrer_group.find(".referrer_group_enabled:checked").length) {
        ...
        referrer_groups
          .not($referrer_group)
          .each(function (index, compare_referrer_group) {
            var $compare_referrer_group = jQuery(compare_referrer_group);

            if (
              $compare_referrer_group.find(".referrer_group_enabled:checked")
                .length
            ) {
              var compare_name = $compare_referrer_group
                .find(".referrer_group")
                .text();
              var compare_theme = $compare_referrer_group.find("select").val();
              var compare_redirect = $compare_referrer_group
                .find("input[type=text]")
                .val();
              var compare_agents = $compare_referrer_group
                .find("textarea")
                .val()
                .split("\n")
                .filter(function (line) {
    ...
    cookiegroups.each(function (index, cookiegroup) {
      ...
      if ($cookiegroup.find(".cookiegroup_enabled:checked").length) {
        ...
        cookiegroups
          .not($cookiegroup)
          .each(function (index, compare_cookiegroup) {
            var $compare_cookiegroup = jQuery(compare_cookiegroup);

            if (
              $compare_cookiegroup.find(".cookiegroup_enabled:checked").length
            ) {
              var compare_name = $compare_cookiegroup
                .find(".cookiegroup_name")
                .text();
              var compare_agents = $compare_cookiegroup
                .find("textarea")
                .val()
                .split("\n")
                .filter(function (line) {
  ...
  jQuery(document).on("click", "#mobile_add", function () {
    var group = prompt(
      'Enter group name (only "0-9", "a-z", "_" symbols are allowed).',
    );
        ...
        if (!exists) {
          var li = jQuery(
            '<li id="mobile_group_' +
              ...
              "<th>Group name:</th>" +
              "<td>" +
              '<span class="mobile_group_number">' +
              (jQuery("#mobile_groups li").length + 1) +
              ".</span> " +
              ...
              "</tr>" +
              "<tr>" +
              '<th><label for="mobile_groups_' +
              group +
              '_enabled">Enabled:</label></th>' +
              "<td>" +
              '<input type="hidden" name="mobile_groups[' +
              group +
              '][enabled]" value="0" />' +
              '<input id="mobile_groups_' +
              group +
              '_enabled" class="mobile_group_enabled" type="checkbox" name="mobile_groups[' +
              group +
              ...
              '<th><label for="mobile_groups_' +
              group +
              '_theme">Theme:</label></th>' +
              "<td>" +
              '<select id="mobile_groups_' +
              group +
              '_theme" name="mobile_groups[' +
              group +
              '][theme]"><option value="">-- Pass-through --</option></select>' +
              ...
              '<th><label for="mobile_groups_' +
              group +
              '_redirect">Redirect users to:</label></th>' +
              "<td>" +
              '<input id="mobile_groups_' +
              group +
              '_redirect" type="text" name="mobile_groups[' +
              group +
              '][redirect]" value="" size="60" />' +
              ...
              '<th><label for="mobile_groups_' +
              group +
              '_agents">User agents:</label></th>' +
              "<td>" +
              '<textarea id="mobile_groups_' +
              group +
              '_agents" name="mobile_groups[' +
              group +
              '][agents]" rows="10" cols="50"></textarea>' +
        ...
        if (!exists) {
          var li = jQuery(
            '<li id="referrer_group_' +
              ...
              "<th>Group name:</th>" +
              "<td>" +
              '<span class="referrer_group_number">' +
              (jQuery("#referrer_groups li").length + 1) +
              ".</span> " +
              '<span class="referrer_group">' +
              group +
              "</span> " +
              ...
              "<tr>" +
              "<th>" +
              '<label for="referrer_groups_' +
              group +
              '_enabled">Enabled:</label>' +
              "</th>" +
              "<td>" +
              '<input type="hidden" name="referrer_groups[' +
              group +
              '][enabled]" value="0" />' +
              '<input id="referrer_groups_' +
              group +
              '_enabled" class="referrer_group_enabled" type="checkbox" name="referrer_groups[' +
              group +
              '][enabled]" value="1" checked="checked" />' +
              "</td>" +
              "</tr>" +
              "<tr>" +
              '<th><label for="referrer_groups_' +
              group +
              '_theme">Theme:</label></th>' +
              "<td>" +
              '<select id="referrer_groups_' +
              group +
              '_theme" name="referrer_groups[' +
              group +
              '][theme]"><option value="">-- Pass-through --</option></select>' +
              ...
              '<input id="referrer_groups_' +
              group +
              '_redirect" type="text" name="referrer_groups[' +
              group +
              '][redirect]" value="" size="60" />' +
              ...
              '<th><label for="referrer_groups_' +
              group +
              '_referrers">Referrers:</label></th>' +
              "<td>" +
              '<textarea id="referrer_groups_' +
              group +
              '_referrers" name="referrer_groups[' +
              group +
              '][referrers]" rows="10" cols="50"></textarea>' +
        ...
        if (!exists) {
          var li = jQuery(
            '<li id="cookiegroup_' +
              ...
              "<th>Group name:</th>" +
              "<td>" +
              '<span class="cookiegroup_number">' +
              (jQuery("#cookiegroups li").length + 1) +
              ".</span> " +
              '<span class="cookiegroup_name">' +
              group +
              "</span> " +
              ...
              "</tr>" +
              "<tr>" +
              '<th><label for="cookiegroup_' +
              group +
              '_enabled">Enabled:</label></th>' +
              "<td>" +
              '<input id="cookiegroup_' +
              group +
              '_enabled" class="cookiegroup_enabled" type="checkbox" name="cookiegroups[' +
              group +
              '][enabled]" value="1" checked="checked" />' +
              "</td>" +
              "</tr>" +
              ...
              '<th><label for="cookiegroup_' +
              group +
              '_cache">Cache:</label></th>' +
              "<td>" +
              '<input id="cookiegroup_' +
              group +
              '_cache" type="checkbox" name="cookiegroups[' +
              group +
              '][cache]" value="1" checked="checked" /></td>' +
              "</tr>" +
              "<tr>" +
              '<th><label for="cookiegroups_' +
              group +
              '_cookies">Cookies:</label></th>' +
              "<td>" +
              '<textarea id="cookiegroups_' +
              group +
              '_cookies" name="cookiegroups[' +
              group +
              '][cookies]" rows="10" cols="50"></textarea>' +
...
function unique_array(array) {
  return jQuery.grep(array, function (el, i) {
    return i === jQuery.inArray(el, array);
  });
}
