...
class Util_PageUrls {
	 ...
	 *
	 * @param int $limit_post_pages Limit for posts/pages. Default is 10.
	 *
	 * @return array
	 */
	public static function get_frontpage_urls( $limit_post_pages = 10 ) {
		...
		if ( ! isset( $frontpage_urls[ $limit_post_pages ] ) ) {
			...
			$full_urls = array_merge( $full_urls, self::get_older_pages( $home_path, $limit_post_pages, 'post' ) );
	 ...
	 * @return array
	 */
	public static function get_postpage_urls( $limit_post_pages = 10 ) {
	 ...
	 * @return array
	 */
	public static function get_cpt_archive_urls( $post_id, $limit_post_pages = 10 ) {
	 ...
	 *
	 * @param unknown $posts_page_uri   Posts page URI.
	 * @param int     $limit_post_pages Limit for posts/pages. Default is 10.
	 * @param string  $post_type        Post Type. Default is post.
	 *
	 * @return array
	 */
	private static function get_older_pages( $posts_page_uri, $limit_post_pages = 10, $post_type = 'post' ) {
		$full_urls          = array();
	 ...
	 * @return array
	 */
	public static function get_post_urls( $post_id ) {
	 ...
	 * @return array
	 */
	public static function get_post_comments_urls( $post_id ) {
	 ...
	 *
	 * @param unknown $post_author      Post author.
	 * @param int     $limit_post_pages Page/post limit. Default is 10.
	 *
	 * @return array
	 */
	 ...
	 *
	 * @param unknown $post             Post.
	 * @param int     $limit_post_pages Page/post limit. Default is 10.
	 *
	 * @return array
	 */
	public static function get_daily_archive_urls( $post, $limit_post_pages = 10 ) {
	 ...
	 * @return array
	 */
	public static function get_monthly_archive_urls( $post, $limit_post_pages = 10 ) {
	...
	public static function get_pages_urls( $pages ) {
		...
		if ( ! isset( $pages_urls[ $w3tc_key ] ) ) {
			...
			foreach ( $pages as $uri ) {
				if ( $uri ) {
					$page_link   = get_home_url() . '/' . trim( $uri, '/' ) . ( strpos( $uri, '?' ) !== false ? '' : '/' );
					$full_urls[] = $page_link;
				}
			}
	 ...
	 *
	 * @param unknown $pagenum Page number.
	 *
	 * @return string
	 */
	public static function get_pagenum_link_admin( $pagenum ) {
		global $wp_rewrite;
	 ...
	 * @param integer $max_page Max page.
	 *
	 * @return string
	 */
	public static function get_comments_pagenum_link( $post_id, $pagenum = 1, $max_page = 0 ) {
	 ...
	 * @return array
	 */
	public static function get_rest_posts_urls() {
	 ...
	 * @return array
	 */
	public static function get_rest_post_urls( $post_id ) {
		static $post_urls = array();
	 ...
	 *
	 * @param array $queued_urls Queued URLs.
	 *
	 * @return string
	 */
	 ...
	 * @return string
	 */
	private static function _term_hash( $terms ) {
		$term_hash = array();
		foreach ( $terms as $term ) {
	 ...
	 * @return string
	 */
	private static function _get_archive_slug( $post ) {
		...
		if ( $args->has_archive ) {
			...
			$archive_slug = true === $args->has_archive ? $args->rewrite['slug'] : $args->has_archive;
	...
	public static function get_page_mapping( $id ) {
		$map = array(
			...
			'w3tc_extension_page_imageservice' => array(
				'page_name'   => esc_html__( 'Image Converter', 'w3-total-cache' ),
