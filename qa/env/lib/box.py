...
def init(ip, box_name, box_instance_name):
	...
	shell.scp('', './boxes/' + box_name + '/export.sh', root_ip, '/root/box-environment.sh')
	...
	shell.scp('', './boxes/' + box_name + '/environments/*', root_ip, '/share/environments')
...
#		root_ip, '/share/scripts/w3test')
	...
	shell.ssh(
		root_ip,
		'cd /share; ' +
		'wget ' + vars['W3D_W3TC_ZIP_URL'] + ' -q -O ./w3tc.zip; ' +
		'unzip -q /share/w3tc.zip; ' +
		'\cp -prf ./w3tc/qa/env/scripts/init-box ./scripts/; ' +
