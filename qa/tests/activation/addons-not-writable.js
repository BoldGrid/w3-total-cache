...
describe("", function () {
  ...
  it("configure via FTP after activation", async () => {
    ...
    expect(note).contains(
      "Required files and directories have been automatically created",
    );
  });
...
async function fillFtpForm() {
  ...
  await adminPage.evaluate(
    (showFtpForm) => document.querySelector(showFtpForm).click(),
    showFtpForm,
  );
  await adminPage.waitForSelector("form[name=w3tc_ftp_form]", {
    visible: true,
  });
  await adminPage.$eval("input[name=hostname]", (e) => (e.value = "localhost"));
  await adminPage.$eval("input[name=username]", (e) => (e.value = "ubuntu"));
  ...
  await Promise.all([
    adminPage.evaluate(
      (upgradeButton) => document.querySelector(upgradeButton).click(),
      upgradeButton,
    ),
