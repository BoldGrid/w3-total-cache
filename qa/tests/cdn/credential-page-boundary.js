...
describe("rt9-210 CDN credential page-boundary regression", function () {
  ...
  it("legitimate save from w3tc_cdn writes cdn.s3.key", async () => {
    ...
    expect(await w3tc.getConfigOption("cdn.s3.bucket")).equals(
      "w3tc-qa-bucket-rt9-210",
    );
  ...
  it("cross-page POST of cdn__s3__key is dropped before config write", async () => {
    ...
    let generalNonce = await adminPage.$eval(
      "input[name=_wpnonce]",
      (e) => e.value,
    );
    ...
    let result = await adminPage.evaluate(
      async function (networkAdminUrl, nonce, attackKey) {
        let body = new URLSearchParams();
        body.append("_wpnonce", nonce);
        body.append(
          "_wp_http_referer",
          "/wp-admin/network/admin.php?page=w3tc_general",
        );
