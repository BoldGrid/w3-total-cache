...
describe('check that browsercache doesnt break CDN canonical header', function() {
	...
	it('set options', async() => {
		...
		await w3tc.setOptions(adminPage, 'w3tc_browsercache', {
			...
			browsercache_other_cache_policy: 'cache'
		});
...
function expectCommonHeaders(headers) {
	...
	expect(headers['expires']).not.empty;
	expect(headers['link'].indexOf('>; rel="canonical"') >= 0).is.true;
	expect(headers['pragma']).equals('public');
