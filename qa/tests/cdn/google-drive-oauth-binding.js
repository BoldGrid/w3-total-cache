...
async function inlineScriptText(pPage) {
  return pPage.$$eval("script:not([src])", (els) =>
...
describe("rt9-233 Google Drive OAuth state-binding regression", function () {
  ...
  it("CDN page render embeds a state token in the OAuth return_url", async () => {
    ...
    let m = html.match(/w3tc_gdrive_state(?:=|%3D)([a-zA-Z0-9]{32})/);
  ...
  it("crafted oa_* params without state do NOT enqueue auth popup", async () => {
    let craftedUrl =
      env.networkAdminUrl +
      "admin.php?page=w3tc_cdn" +
      "&oa_client_id=" +
      encodeURIComponent(ATTACKER_CLIENT_ID) +
      "&oa_access_token=" +
      encodeURIComponent(ATTACKER_ACCESS_TOKEN) +
      "&oa_refresh_token=" +
      encodeURIComponent(ATTACKER_REFRESH);
  ...
  it("direct GET to auth-return handler without state returns 403", async () => {
    ...
    let response = await adminPage.goto(directUrl, {
      waitUntil: "domcontentloaded",
    });
