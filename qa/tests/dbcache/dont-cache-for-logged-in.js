...
describe('"Dont cache for logged in" works', function () {
  ...
  it("set options", async () => {
    ...
    await sys.copyPhpToRoot(
      "../../plugins/dbcache/dont-cache-for-logged-in.php",
    );
  });
  ...
  it("checking that is not cached for logged in", async () => {
    ...
    await getCache(
      adminPage,
      "test_update",
      "Title updated, the page is not cached",
    );
  });
  ...
  it("check that cache is used when switched off", async () => {
    ...
    await getCache(
      adminPage,
      "test_update",
      "The cache was potentially created",
    );
    await updateOutOfWP("Hello");
    await getCache(
      adminPage,
      "test_update",
      "Title was not updated, the cache retrieved",
    );
  });
});
...
async function getCache(pPage, title, text) {
  await pPage.goto(
    env.blogSiteUrl +
      "dont-cache-for-logged-in.php?action=get_cache&title=" +
      title +
      "&engine=" +
...
async function updateOutOfWP(title) {
  await page.goto(
    env.blogSiteUrl +
      "dont-cache-for-logged-in.php?action=update_record&title=" +
      title +
      "&engine=" +
      env.cacheEngineLabel,
  );
