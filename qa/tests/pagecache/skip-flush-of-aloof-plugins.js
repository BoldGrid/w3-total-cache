...
describe("", function () {
  ...
  it("copy theme files", async () => {
    ...
    await sys.copyPhpToRoot(
      "../../plugins/pagecache/skip-flush-of-aloof-plugins-action.php",
    );

    await sys.copyPhpToPath(
      "../../plugins/pagecache/skip-flush-of-aloof-plugins.php",
  ...
  it("check", async () => {
    ...
    log.log(
      'Going to the homepage to check if the file has "test of cache" text...',
    );
  ...
  it("flush via button", async () => {
    if (env.isWpmu) {
      ...
      await Promise.all([
        ...
        adminPage.evaluate(
          (pgcacheFlushButton) =>
            document.querySelector(pgcacheFlushButton).click(),
          pgcacheFlushButton,
        ),
      ]);
  ...
  it("flush via w3tc_flush_all", async () => {
    let controlUrl =
      env.blogSiteUrl +
      "skip-flush-of-aloof-plugins-action.php?action=w3tc_flush_all";
  ...
  it("flush via w3tc_flush_posts", async () => {
    let controlUrl =
      env.blogSiteUrl +
      "skip-flush-of-aloof-plugins-action.php?action=w3tc_flush_posts";
    await adminPage.goto(controlUrl, { waitUntil: "domcontentloaded" });
    let html = await adminPage.content();
    expect(html).contains("ok");
