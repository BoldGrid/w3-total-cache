...
describe("", function () {
  ...
  it("copy qa files", async () => {
    await sys.copyPhpToPath(
      "../../plugins/pagecache/user-roles-single.php",
  ...
  it("relogin to issue new cookies", async () => {
    let logoutUrl = await adminPage.$eval(
  ...
  it("page not cached for the administrator", async () => {
    ...
    expect(await adminPage.content()).matches(
      new RegExp(
        "Page Caching using .+? \\(Rejected user role is logged in\\)",
      ),
    );
  ...
  it("user signup", async () => {
    ...
    await Promise.all([
      page.evaluate(
        (wpSubmit) => document.querySelector(wpSubmit).click(),
        wpSubmit,
      ),
