...
describe("UserExperience: Delay Scripts (Pro) form-save", function () {
  ...
  it("Pro-only Delay Scripts inputs round-trip when Pro is active", async function () {
     ...
     */
    await w3tc
      .activateExtension(adminPage, "user-experience-defer-scripts")
    ...
    if (inputEl === null) {
      log.log(
        "SKIP: Delay Scripts input not rendered (free build or extension off)",
      );
      this.skip();
      return;
    }
    ...
    await w3tc.setOptions(adminPage, "w3tc_userexperience", {
      ...
      "user-experience-defer-scripts___includes":
        "googletagmanager.com\ngtag/js",
      "user-experience-defer-scripts___excludes":
        "jquery.js\nlogged-in-only.js",
    });
