...
describe("UserExperience: Lazy Load settings round-trip", function () {
  ...
  it("save and read back every Lazy Load setting", async () => {
    ...
    await adminPage.goto(
      env.networkAdminUrl + "admin.php?page=w3tc_userexperience",
      { waitUntil: "domcontentloaded" },
    );

    let processImg = await adminPage
      .$eval("#lazyload__process_img", (e) => e.checked)
      .catch(() => null);
    expect(processImg).equals(true);

    let processBg = await adminPage
      .$eval("#lazyload__process_background", (e) => e.checked)
      .catch(() => null);
    expect(processBg).equals(true);

    let excludeVal = await adminPage
      .$eval("#lazyload__exclude", (e) => e.value)
      .catch(() => "");
    expect(excludeVal).contains("no-lazy");
    expect(excludeVal).contains("themehero");

    let thresholdVal = await adminPage
      .$eval("#lazyload__threshold", (e) => e.value)
      .catch(() => "");
    expect(thresholdVal).equals("300px");

    let embedSelect = await adminPage
      .$eval("#lazyload__embed_method", (e) => e.value)
      .catch(() => "");
  ...
  it("disabling the master toggle persists", async () => {
    ...
    let enabled = await adminPage
      .$eval("#lazyload__enabled", (e) => e.checked)
      .catch(() => null);
