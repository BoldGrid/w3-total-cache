...
let redirectedUrl =
  env.scheme +
  "://for-tests.sandbox" +
  (env.scheme == "http" && env.httpServerPort != 80
    ? ":" + env.httpServerPort
    : "") +
  (env.scheme == "https" && env.httpServerPort != 443
    ? ":" + env.httpServerPort
    : "") +
  "/";
let pluginUrl =
  env.blogSiteUrl.replace(/(b2\.)?wp\.sandbox/i, "for-tests.wp.sandbox") +
  "referrer-groups.php?path=" +
  env.blogSiteUrl;
...
describe("", function () {
  ...
  it("not redirected to http://for-tests.sandbox", async () => {
    ...
    await Promise.all([
      page.evaluate(
        (helloWorld) => document.querySelector(helloWorld).click(),
        helloWorld,
      ),
  ...
  it("add referrer group", async () => {
    ...
    await adminPage.$eval(
      "#referrer_groups_test_group_referrers",
      (e) => (e.value = "for-tests\\.wp\\.sandbox(\\/wp\\/)?"),
    );
    await adminPage.$eval(
      "#referrer_groups_test_group_redirect",
      (e, v) => (e.value = v),
      redirectedUrl,
    );
