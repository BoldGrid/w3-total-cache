...
async function testWithDifferentOptions(oc, db, autoload) {
  log.log(
    'Testing Object Cache set to "' +
      oc +
      '" and Database Cache set to "' +
      db +
      '" with options autoload set to "' +
      autoload +
      '"...',
  );
  ...
  await page.goto(
    pluginFileUrl + "?action=add_option&value=test1&autoload=" + autoload,
  );
...
async function flushObjectCache() {
  ...
  let flushLink = await adminPage.$eval(
    "#wp-admin-bar-w3tc_flush_objectcache a",
    (e) => e.href,
  );
  await adminPage.goto(flushLink);

  let content = await adminPage.$eval(
    "#flush_objectcache",
    (e) => e.textContent,
  );
...
async function flushDBCache() {
  ...
  let flushLink = await adminPage.$eval(
    "#wp-admin-bar-w3tc_flush_dbcache a",
    (e) => e.href,
  );
  await adminPage.goto(flushLink);
