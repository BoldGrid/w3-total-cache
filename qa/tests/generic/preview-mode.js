...
describe("", function () {
  ...
  it("enable preview mode", async () => {
    ...
    await Promise.all([
      adminPage.evaluate(
        (configPreviewEnable) =>
          document.querySelector(configPreviewEnable).click(),
        configPreviewEnable,
      ),
    ...
    expect(await adminPage.content()).contains(
      "Preview mode was successfully enabled",
    );
  });
  ...
  it('"deploy" changes in preview mode to usual', async () => {
    ...
    await Promise.all([
      adminPage.evaluate(
        (deployButton) => document.querySelector(deployButton).click(),
        deployButton,
      ),
      adminPage.waitForNavigation(),
    ]);
    expect(await adminPage.content()).contains(
      "Preview settings successfully deployed.",
    );
  });
