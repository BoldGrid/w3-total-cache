...
async function isDashboardVisible(page) {
  return page
    .$eval("#w3tc-wizard-dashboard", (e) => e.offsetParent !== null)
    .catch(() => false);
}

async function isNextEnabled(page) {
  return page
    .$eval("#w3tc-wizard-next", (e) => !e.disabled && e.offsetParent !== null)
    .catch(() => false);
}
...
describe("Setup Guide wizard happy path", function () {
  ...
  it("admin can walk the wizard from welcome to dashboard", async () => {
    ...
    await adminPage.goto(
      env.networkAdminUrl + "admin.php?page=w3tc_setup_guide",
      { waitUntil: "networkidle0", timeout: 60000 },
    );

    // First slide must mount.
    await adminPage.waitForSelector("#w3tc-wizard-container", {
      timeout: 30000,
    });
    ...
    if (tosPresent) {
      ...
      await adminPage.evaluate(() => {
        document
          .querySelector('#w3tc-licensing-terms .button[data-choice="accept"]')
          .click();
      });
    ...
    for (let step = 0; step < MAX_STEPS; step++) {
      ...
      if (!(await isNextEnabled(adminPage))) {
        log.log(
          "Next button not interactable at step " +
            step +
