 ...
 */
const XSS_AGENT_PAYLOAD =
  "<script>window.__w3tc_cachegroups_xss_fired=1</script>iPhone";
...
describe("sec-xss CacheGroups UserAgent regression", function () {
  ...
  it("benign user-agent group saves and re-renders", async () => {
    ...
    let r1 = await adminPage.evaluate(
      async function (adminUrl, n) {
    ...
    let benignAgents = await adminPage
      .$eval("#mobile_groups_benign_agents", (e) => e.value)
      .catch(() => "");
  ...
  it("<script> payload in agent body is stripped + escaped on render", async () => {
    ...
    let r1 = await adminPage.evaluate(
      async function (adminUrl, n, payload, groupName) {
        let body = new URLSearchParams();
        body.append("_wpnonce", n);
        body.append("w3tc_save_options", "Save all settings");
     ...
     */
    let agentVal = await adminPage
      .$eval("#mobile_groups_" + XSS_GROUP_NAME + "_agents", (e) => e.value)
      .catch(() => "");
    ...
    expect(pageHtml).not.contains(
      "<script>window.__w3tc_cachegroups_xss_fired",
    );
