...
describe("", function () {
  ...
  it("check", async () => {
    ...
    expect(await page.content()).contains(
      '"value":"existing-value","found":true',
    );

    log.log("getFromCache non existing-value");
    await page.goto(
      env.blogSiteUrl +
        "basic.php?group=group-existing&id=object-non-existing&action=getFromCache",
    );
  ...
  it("check groups", async () => {
    ...
    log.log("Get object1...");
    await page.goto(
      env.blogSiteUrl + "basic.php?group=group1&id=object1&action=getFromCache",
    );
    expect(await page.content()).contains('"value":"test1","found":true');

    log.log("Get object2...");
    await page.goto(
      env.blogSiteUrl + "basic.php?group=group2&id=object2&action=getFromCache",
    );
    expect(await page.content()).contains('"value":"test2","found":true');

    log.log("Flush group1...");
    await page.goto(
      env.blogSiteUrl + "basic.php?group=group1&action=flushGroup",
    );
    ...
    log.log("Flush...");
    await page.goto(env.blogSiteUrl + "basic.php?action=flush");
