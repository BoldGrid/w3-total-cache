...
describe("", function () {
  ...
  it("configure minify", async () => {
    let themesPath =
      env.wpContentPath.replace(/\/var\/www\/wp-sandbox/g, "") +
      "themes/" +
      theme;
    ...
    await w3tc.setOptionsMinifyAddJsEntry(
      adminPage,
      1,
      wpIncludesPath + "/js/jquery/jquery.js",
      "include",
    );
    await w3tc.setOptionsMinifyAddJsEntry(
      adminPage,
      2,
      wpIncludesPath + "/js/jquery/jquery-migrate.min.js",
      "include",
    );
    await w3tc.setOptionsMinifyAddJsEntry(
      adminPage,
      3,
      themesPath + "/qa/minify-js1.js",
      "include",
    );
    await w3tc.setOptionsMinifyAddJsEntry(
      adminPage,
      4,
      themesPath + "/qa/minify-js2.js",
      "include",
    );
    await w3tc.setOptionsMinifyAddJsEntry(
      adminPage,
      5,
      themesPath + "/qa/minify-non-blocking-js3.js",
      "include",
    );
    await w3tc.setOptionsMinifyAddJsEntry(
      adminPage,
      6,
      themesPath + "/qa/minify-js4.js",
      "include-body",
    );
    await w3tc.setOptionsMinifyAddJsEntry(
      adminPage,
      7,
      themesPath + "/qa/minify-js5.js",
      "include-body",
    );
    await w3tc.setOptionsMinifyAddJsEntry(
      adminPage,
      8,
      themesPath + "/qa/minify-js6.js",
      "include-footer",
    );

    await w3tc.setOptionsMinifyAddCssEntry(
      adminPage,
      1,
      themesPath + "/qa/minify-style.css",
      "include",
    );
...
async function checkPlacement(comments, helperComments, cssOrJs) {
  ...
  if (cssOrJs == "css") {
    reg = new RegExp(
      '<\\!-- test css placement --><link rel="stylesheet"' +
        '[^>]+href="[^"]+\\/cache\\/minify.+\\.css"',
      "gm",
    );
  } else {
    reg = new RegExp(
  ...
  expect(
    minifiedComments,
    `expected a minified ${cssOrJs} placement for "${comments}" matching ${reg}`,
  ).is.not.null;
