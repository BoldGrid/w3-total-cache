...
describe("", function () {
  ...
  it("configure minify", async () => {
    ...
    let valueOriginal =
      "//fonts.googleapis.com/css?family=Ubuntu%3A400%2C700%26subset%3Dlatin%2Clatin-ex";
    ...
    let valueAfterSave = await adminPage.$eval(
      "#css_files input[type=text].css_enabled",
      (e) => e.value,
    );
  ...
  it("font was minified", async () => {
    ...
    var css = expect(html).matches(
      /https?:\/\/.+?\/cache\/minify\/(\d+\/)?.+?default\.include\..+?\.css/g,
    );
  });
});
