...
describe("minify html", function () {
  ...
  it("copy theme files", async () => {
    ...
    await sys.copyPhpToPath(
      "../../plugins/minify-auto-theme/*",
      `${themePath}/qa`,
    );
    await wp.addQaBootstrap(
      adminPage,
      `${themePath}/functions.php`,
      "/qa/minify-html-sc.php",
    );
  });
  ...
  it("whitespace compression", async () => {
    ...
    let dataAttr = await page.$eval("#multiple-whitespace1", (e) =>
    ...
    expect(html).equals(
      '<div id="multiple-whitespace1" data-attr="   space-in-attr   "> multiple whitespace around element</div>',
    );

    expect(testPageHtml).contains(
  ...
  it("trailing slash removal on void elements", async () => {
    ...
    let e5input = await page.$eval(
      '#void-elements5 input[value="5"]',
      (e) => e.type,
    );
  ...
  it("attribute quotes removal", async () => {
    let e1class = await page.$eval("#quoted-attribute1", (e) =>
    ...
    let e2q1 = await page.$eval("#quoted-attribute2", (e) =>
      e.getAttribute("data-test-quote1"),
    );
    expect(e2q1).empty;
    let e2q2 = await page.$eval("#quoted-attribute2", (e) =>
      e.getAttribute("data-test-quote2"),
    );
    expect(e2q2).empty;
    // spaces, zeroes not removed
    let e2q3 = await page.$eval("#quoted-attribute2", (e) =>
      e.getAttribute("data-test-quote3"),
    );
    expect(e2q3).equals("0");
    let e2q4 = await page.$eval("#quoted-attribute2", (e) =>
      e.getAttribute("data-test-quote4"),
    );
    expect(e2q4).equals(" ");

    // html view is correct - no value
    expect(testPageHtml).contains(
      "id=quoted-attribute2 data-test-quote1 data-test-quote2 data-test-quote3=0 data-test-quote4=' '>",
    );

    let e3q1 = await page.$eval("#quoted-attribute3", (e) =>
      e.getAttribute("data-test-quote1"),
    );
    expect(e3q1).equals("'");
    let e3q2 = await page.$eval("#quoted-attribute3", (e) =>
      e.getAttribute("data-test-quote2"),
    );
    expect(e3q2).equals('"');

    let mixed1q1 = await page.$eval("#quoted-attribute-mixed1", (e) =>
      e.getAttribute("data-test-quote1"),
    );
    expect(mixed1q1).equals("' at start");
    let mixed1q2 = await page.$eval("#quoted-attribute-mixed1", (e) =>
      e.getAttribute("data-test-quote2"),
    );
    expect(mixed1q2).equals("at ' middle");
    let mixed1q3 = await page.$eval("#quoted-attribute-mixed1", (e) =>
      e.getAttribute("data-test-quote3"),
    );
    expect(mixed1q3).equals("at end'");

    let mixed2q1 = await page.$eval("#quoted-attribute-mixed2", (e) =>
      e.getAttribute("data-test-quote1"),
    );
    expect(mixed2q1).equals('" at start');
    let mixed2q2 = await page.$eval("#quoted-attribute-mixed2", (e) =>
      e.getAttribute("data-test-quote2"),
    );
    expect(mixed2q2).equals('at " middle');
    let mixed2q3 = await page.$eval("#quoted-attribute-mixed2", (e) =>
      e.getAttribute("data-test-quote3"),
    );
