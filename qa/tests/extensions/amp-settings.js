...
describe("AMP extension settings form-save", function () {
  ...
  it("activate AMP and round-trip url_type + url_postfix", async function () {
    await w3tc
      .activateExtension(adminPage, "amp")
    ...
    let postfix = await adminPage
      .$eval("#amp___url_postfix", (e) => e.value)
      .catch(() => "");
     ...
     */
    nonce = await adminPage
      .$eval("input[name=_wpnonce]", (e) => e.value)
      .catch(() => "");
    let r2 = await adminPage.evaluate(
      async function (url, n) {
        let body = new URLSearchParams();
        body.append("_wpnonce", n);
        body.append("w3tc_save_options", "Save all settings");
