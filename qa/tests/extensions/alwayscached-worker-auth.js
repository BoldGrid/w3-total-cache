...
describe('AlwaysCached worker HTTP-trigger auth gate', function() {
	...
	let triggerUrl = env.homeUrl + (env.homeUrl.indexOf('?') !== -1 ? '&' : '?') +
		'w3tc_alwayscached=1';
	...
	before(async function() {
		...
		let r = await exec(
			'sudo -u www-data wp option get w3tc_config --format=json --path=' + env.wpPath +
			' 2>/dev/null || echo "{}"');
		let blob = {};
		try { blob = JSON.parse(r.stdout); } catch (e) {}
		let active = blob['extensions.active'] || {};
		if (typeof active.alwayscached === 'undefined') {
			log.log('SKIP: alwayscached extension not in extensions.active (free build?)');
			this.skip();
		}
	...
	it('anon GET to ?w3tc_alwayscached returns 403 Forbidden', async() => {
		...
		log.log('anon ?w3tc_alwayscached -> ' + r.statusCode +
			' body[0..120]=' + JSON.stringify(r.body.substring(0, 120)));
	...
	it('anon GET with correct W3TC_WORKER_SECRET is accepted', async function() {
		 ...
		 */
		let cmd = "sudo grep -q '" + probeMarker + "' " + wpConfigPath + " || " +
			"sudo sed -i \"/That's all, stop editing/i define( 'W3TC_WORKER_SECRET', '" +
			KNOWN_SECRET + "' ); " + probeMarker + "\" " + wpConfigPath;
		try {
			await exec(cmd);
		} catch (e) {
			log.log('wp-config patch error (may be OK on read-only matrix): ' + e.message);
			this.skip();
		}
		...
		log.log('correct-bearer -> ' + r.statusCode +
			' body[0..120]=' + JSON.stringify(r.body.substring(0, 120)));
	...
	it('admin browser session can trigger the worker', async() => {
		 ...
		 */
		let resp = await adminPage.goto(triggerUrl, {waitUntil: 'domcontentloaded'});
		let body = await adminPage.content();
		log.log('admin GET status: ' + resp.status() +
			' body[0..200]=' + JSON.stringify(body.substring(0, 200)));
