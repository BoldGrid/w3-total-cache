...
function alwaysCachedSettingsPresent(html) {
  return (
    html.indexOf("Enable WP-Cron Event") !== -1 ||
...
describe("AlwaysCached extension settings form-save", function () {
  ...
  it("save and read back AlwaysCached cron settings", async function () {
    ...
    let saveStatus = await adminPage.evaluate(
    ...
    let cronEnabled = await adminPage
      .$eval("#alwayscached__wp_cron", (e) => e.checked)
      .catch(() => null);
    expect(cronEnabled).equals(true);

    let cronTime = await adminPage
      .$eval("#alwayscached__wp_cron_time", (e) => e.value)
      .catch(() => "");
    expect(cronTime).equals("0");

    let cronInterval = await adminPage
      .$eval("#alwayscached__wp_cron_interval", (e) => e.value)
      .catch(() => "");
