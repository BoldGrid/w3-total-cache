...
describe("Swarmify extension settings form-save", function () {
  ...
  it("activate Swarmify and round-trip every config key", async function () {
    await w3tc
      .activateExtension(adminPage, "swarmify")
    ...
    let nonce = await adminPage
      .$eval("input[name=_wpnonce]", (e) => e.value)
      .catch(() => "");
    if (!nonce) {
    ...
    let apiKey = await adminPage
      .$eval("#swarmify___api_key", (e) => e.value)
      .catch(() => "");
    expect(apiKey).equals("qa-swarmify-api-key-12345");

    let htmlVideo = await adminPage
      .$eval("#swarmify___handle__htmlvideo", (e) => e.checked)
      .catch(() => null);
    expect(htmlVideo).equals(true);

    let jwplayer = await adminPage
      .$eval("#swarmify___handle__jwplayer", (e) => e.checked)
      .catch(() => null);
    expect(jwplayer).equals(true);

    let rejectLogged = await adminPage
      .$eval("#swarmify___reject__logged", (e) => e.checked)
      .catch(() => null);
