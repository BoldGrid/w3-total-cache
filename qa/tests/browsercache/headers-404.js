...
describe("", function () {
  ...
  it("set options", async () => {
    ...
    await w3tc.setOptions(adminPage, "w3tc_browsercache", {
      ...
      browsercache_no404wp_exceptions: w3tc.regExpForOption(
        env.blogHomeUri + "404test.jpg",
      ),
    });
  ...
  it("check 404 pages", async () => {
    ...
    log.log(
      "check that 404 is returned by wp for exception url (not http server)",
    );
    await page.goto(env.homeUrl + "404test.jpg", {
      waitUntil: "domcontentloaded",
    });
