...
describe("", function () {
  ...
  it("find minify assets generated", async () => {
    ...
    let response = await page.goto(scripts[0], {
  ...
  it("check that cached url structure changed", async () => {
    ...
    if (minifiedScripts.length === 0) {
      log.log(
        "no minified scripts found on page - skipping rewrite URL structure check",
      );
    ...
    if (!id) {
      log.log(
        "no scripts found with rewritten URL format (.x[0-9]{5}.js) - browser cache rewrite may not be working",
      );
      // This is a failure case - rewrite should be working
      throw new Error(
        "Browser cache rewrite is enabled but no scripts have rewritten URL format",
      );
    }
    ...
    let response = await page.goto(foundScript, {
      waitUntil: "domcontentloaded",
    });
    expect(response.status() < 400);
  });
