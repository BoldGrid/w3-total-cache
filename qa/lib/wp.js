...
async function getCurrentTheme(pPage) {
  await pPage.goto(env.adminUrl + "themes.php", {
    waitUntil: "domcontentloaded",
  });

  let theme = await pPage.$eval(".theme.active", (e) =>
...
async function postCreate(pPage, data) {
  ...
  let r = await exec(
    "cp ../../plugins/w3tcqa-json.php " + env.wpPath + "w3tcqa-json.php",
  );
  ...
  let controlUrl =
    ...
    "&body=" +
    encodeURIComponent(JSON.stringify(apiBody));
  ...
  let resultString = await pPage.$eval("#result", (e) => {
    return e.value;
  });
  let result;
  try {
  ...
  let controlUrl2 =
    ...
    "w3tcqa-json.php" +
    "?url=" +
    encodeURIComponent(apiUrl2) +
    "&body=" +
    encodeURIComponent(JSON.stringify(apiBody2));
  log.log(`opening ${controlUrl2}`);
  await pPage.goto(controlUrl2, { waitUntil: "domcontentloaded" });
  await pPage.waitForSelector("#resultReady", { visible: true });
  let resultString2 = await pPage.$eval("#result", (e) => {
    return e.value;
  });
...
async function postUpdate(pPage, data) {
  ...
  await pPage.goto(
    env.adminUrl + "post.php?post=" + data.post_id + "&action=edit",
    { waitUntil: "domcontentloaded" },
  );
  log.log(
...
async function addWpConfigConstant(pPage, name, value) {
  ...
  for (let n = 0; n < 100; n++) {
    ...
    log.log(html);
    log.log(
      "constant is still not defined - waiting PHP to catch filesystem updates",
    );
...
async function networkActivatePlugin(pPage, pluginFilename) {
  ...
  await Promise.all([
    pPage.evaluate(
      (pluginActivate) => document.querySelector(pluginActivate).click(),
      pluginActivate,
    ),
...
async function userSignUpSingle(pPage, data) {
  ...
  await pPage.evaluate(
    (sendUserNotification) =>
      document.querySelector(sendUserNotification).click(),
    sendUserNotification,
  );

  let generatePw = ".wp-generate-pw";
  await pPage.evaluate(
    (generatePw) => document.querySelector(generatePw).click(),
    generatePw,
  );
  ...
  await Promise.all([
    pPage.evaluate(
      (createUserSub) => document.querySelector(createUserSub).click(),
      createUserSub,
    ),
    pPage.waitForNavigation(),
  ]);
...
async function userSignUpNetwork(pPage, data) {
  ...
  await pPage.evaluate(
    (registration2) => document.querySelector(registration2).click(),
    registration2,
  );
  ...
  let emailContent = await fs.readFileAsync(
    env.wpContentPath + "mail.txt",
    "utf8",
  );
  expect(emailContent).not.empty;
  let emailMatch = emailContent.match(
    new RegExp("http.*wp-activate.php([^< ]+)"),
  );
