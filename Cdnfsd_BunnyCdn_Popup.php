...
class Cdnfsd_BunnyCdn_Popup {
	...
	public static function w3tc_ajax() {
		$w3tc_o = new Cdnfsd_BunnyCdn_Popup();

		\add_action(
			'w3tc_ajax_cdn_bunnycdn_fsd_intro',
			array( $w3tc_o, 'w3tc_ajax_cdn_bunnycdn_fsd_intro' )
		);

		\add_action(
			'w3tc_ajax_cdn_bunnycdn_fsd_list_pull_zones',
			array( $w3tc_o, 'w3tc_ajax_cdn_bunnycdn_fsd_list_pull_zones' )
		);

		\add_action(
			'w3tc_ajax_cdn_bunnycdn_fsd_configure_pull_zone',
			array( $w3tc_o, 'w3tc_ajax_cdn_bunnycdn_fsd_configure_pull_zone' )
		);

		\add_action(
			'w3tc_ajax_cdn_bunnycdn_fsd_deauthorization',
			array( $w3tc_o, 'w3tc_ajax_cdn_bunnycdn_fsd_deauthorization' )
		);

		\add_action(
			'w3tc_ajax_cdn_bunnycdn_fsd_deauthorize',
			array( $w3tc_o, 'w3tc_ajax_cdn_bunnycdn_fsd_deauthorize' )
		);
	}
	...
	public function w3tc_ajax_cdn_bunnycdn_fsd_list_pull_zones() {
		...
		$api                  = new Cdn_BunnyCdn_Api(
			Cdn_BunnyCdn_Api::client_config_from_saved(
				$w3tc_account_api_key,
				$w3tc_config,
		...
		try {
			...
			$this->render_intro(
				array(
					'account_api_key' => empty( $w3tc_account_api_key ) ? null : $w3tc_account_api_key,
					/**
					 * Copilot review (PR #4): supplier passes raw,
					 * the view's sink-side `esc_html()` is the
					 * single escape point. Matches the sec-xss
					 * skill "strip the now-redundant supplier
					 * escapes" rule and avoids double-escape
					 * cosmetics (`&amp;amp;`-style entities) in
					 * the admin UI.
					 */
		...
		$details = array(
			...
			'pull_zone_id'              => $w3tc_config->get_integer( 'cdnfsd.bunnycdn.pull_zone_id' ),
	...
	public function w3tc_ajax_cdn_bunnycdn_fsd_configure_pull_zone() {
		...
		$w3tc_custom_hostnames = explode( ',', Util_Request::get_string( 'custom_hostnames' ) );
		...
		if ( empty( $pull_zone_id ) ) {
			...
			if ( ! empty( $w3tc_custom_hostnames ) ) {

				foreach ( $w3tc_custom_hostnames as $custom_hostname ) {
					try {
		...
		// Save configuration.
		$w3tc_config->set( 'cdnfsd.bunnycdn.pull_zone_id', (int) $pull_zone_id );
		$w3tc_config->set( 'cdnfsd.bunnycdn.name', $w3tc_name );
		$w3tc_config->set( 'cdnfsd.bunnycdn.origin_url', $w3tc_origin_url );
		$w3tc_config->set( 'cdnfsd.bunnycdn.cdn_hostname', $w3tc_cdn_hostname );
		$w3tc_config->save();
	...
	public function w3tc_ajax_cdn_bunnycdn_fsd_deauthorization() {
		$w3tc_config         = Dispatcher::config();
		$w3tc_origin_url     = $w3tc_config->get_string( 'cdnfsd.bunnycdn.origin_url' ); // Origin URL or IP.
		$w3tc_name           = $w3tc_config->get_string( 'cdnfsd.bunnycdn.name' ); // Pull zone name.
		$w3tc_cdn_hostname   = $w3tc_config->get_string( 'cdnfsd.bunnycdn.cdn_hostname' ); // Pull zone CDN hostname.
		$cdn_pull_zone_id    = $w3tc_config->get_string( 'cdn.bunnycdn.pull_zone_id' ); // CDN pull zone id.
		$cdnfsd_pull_zone_id = $w3tc_config->get_string( 'cdnfsd.bunnycdn.pull_zone_id' ); // CDN FSD pull zone id.
