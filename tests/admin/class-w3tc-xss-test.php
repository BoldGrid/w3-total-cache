 ...
 * Class: W3tc_Xss_Test
 *
 * Coverage for the ENG7-3023 output-escaping pass:
 *  - `CacheGroups_Plugin_Admin::clean_values()` strips HTML tags at
 *    the write boundary so a persisted User-Agent / referrer match
 *    string cannot carry an HTML tag through a future render path
 *    that drops `esc_textarea`.
...
class W3tc_Xss_Test extends WP_UnitTestCase {
	...
	/**
	 * The cachegroups User-Agent / referrer sanitiser strips an HTML
	 * tag from a write-time value — even if a future view drops the
