 ...
 *
 * Regressions for the ENG7-3035 header-injection hardening pass:
 ...
 *    invalid name or a poisoned value.
 *
 * @package    W3TC
...
class W3tc_Header_Injection_Test extends WP_UnitTestCase {
	...
	public function test_cdn_engine_enum_accepts_known_slugs() {
		...
		foreach ( array( '', 'bunnycdn', 's3', 'cloudfront', 'akamai', 'rackspace_cdn' ) as $engine ) {
	...
	public function test_cdn_engine_enum_rejects_unknown_slug() {
		...
		// CRLF-bearing payload that would have split the X-W3TC-CDN response header.
		$payload = "bunnycdn\r\nSet-Cookie: pwned=1";
		$config->set( 'cdn.engine', $payload );
