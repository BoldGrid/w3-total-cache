 ...
 *
 * Regressions for the ENG7-3027 info-leak hardening pass:
 *  - Util_Debug::redact_wpnonce stops at whitespace (was `[^&\]]`,
 *    which kept the value visible past spaces, tabs, newlines).
...
class W3tc_Info_Leak_Test extends WP_UnitTestCase {
	 ...
	 * `&`, but the old pattern (`[^&\]]+`) ran on past whitespace and
	 * leaked the nonce into adjacent log context.
	 *
	 * @since X.X.X
	 */
...
PHP;
	...
	/**
	 * Non-string input returns an empty string rather than blowing up.
	 *
	 * @since X.X.X
	 */
	public function test_redact_secrets_non_string_input() {
		...
		$this->assertSame( '', Util_Debug::redact_secrets( 42 ) );
	}
