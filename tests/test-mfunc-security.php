 ...
 * (PgCache_ContentGrabber) against every meaningful morphed-tag variant.
 *
...
/**
 * 1a. Morphed no-space tag (token embedded in the tag name):
...
/**
 * ---------------------------------------------------------------------------
 * ── SECTION 3: End-to-end simulation of the full submit-to-serve chain
 * ──   Simulates: submit → sanitize → store → serve from cache → parse
 * ---------------------------------------------------------------------------
 */

echo "\n=== END-TO-END SUBMIT-TO-SERVE CHAIN ===\n\n";

// Step 1: Submitted content contains the morphed mfunc payload.
...
/**
 * ---------------------------------------------------------------------------
 * ── SECTION 4: Registered-callback dispatcher + HMAC envelope
 * ──   Exercises the actual PgCache_ContentGrabber methods, not just regex.
 * ---------------------------------------------------------------------------
 */

echo "\n=== DISPATCHER + HMAC (PgCache_ContentGrabber) ===\n\n";
...
if ( ! function_exists( 'wp_salt' ) ) {
	 ...
	 * `$GLOBALS['__test_wp_salt']` so the cross-boundary test (4n.bound)
	 * can swap it between sign and dispatch and prove the HMAC key
	 * derivation does NOT consult `wp_salt()`. See PR #1 review (cssjoe):
	 * `wp_salt()` is undefined when `advanced-cache.php` runs, so the
	 * verifier must derive its key from AUTH_KEY/AUTH_SALT directly.
	 */
	$GLOBALS['__test_wp_salt'] = 'test-mfunc-salt-auth';
	function wp_salt( $scheme = 'auth' ) {
		return $GLOBALS['__test_wp_salt'];
	}
}
/**
 * AUTH_KEY / AUTH_SALT are the canonical inputs to `_dynamic_hmac_key()`
 * after the cross-boundary fix. Defining them here makes the standalone
 * suite behave like a real WordPress install for the HMAC tests; the
 * existing PgCache regex tests do not consult them.
 */
if ( ! defined( 'AUTH_KEY' ) ) {
	define( 'AUTH_KEY', 'test-auth-key-d6f04c01' );
}
if ( ! defined( 'AUTH_SALT' ) ) {
	define( 'AUTH_SALT', 'test-auth-salt-1f9a2bb7' );
}
if ( ! defined( 'ABSPATH' ) ) {
...
// ── 4c. mclude with a file-path payload is refused ──
$reset_callbacks();
...
// ── 4h. mclude with valid HMAC + UNREGISTERED slug is refused ──
$reset_callbacks();
$register_callback( 'other_cb', function ( $args ) {
	return 'OTHER';
} );
...
/**
 * ── 4n.bound. Cross-boundary HMAC stability: sign-time vs verify-time.
 * Regression for PR #1 cssjoe review (`_dynamic_hmac_key` divergence).
 * On the regular cache-HIT path for every non-`file_generic` cache
 * engine (Disk: Basic, memcached, redis, APC, XCache), the verifier
 * runs from `advanced-cache.php` BEFORE `wp-includes/pluggable.php`
 * loads — so `wp_salt()` is undefined at verify-time. The signer
 * runs from `ob_callback()` AFTER pluggable is loaded, where
...
$bound_args = '{"name":"alice"}';

// Sign while `wp_salt()` returns one value (mimics post-pluggable ob_callback).
$GLOBALS['__test_wp_salt'] = 'sign-side-pluggable-value';
$bound_hmac                = $grabber->_dynamic_hmac( 'mfunc', 'render_user', $bound_args );
$bound_tag                 = '<!-- mfunc ' . $token . ' call:render_user ' . $bound_args . ' hmac:' . $bound_hmac . ' --><!-- /mfunc ' . $token . ' -->';

/**
 * Dispatch while `wp_salt()` would return a completely different value (mimics
 * advanced-cache.php pre-pluggable, where in production `wp_salt()` does not
 * exist at all). If `_dynamic_hmac_key()` consults `wp_salt()`, the verifier
 * computes a different key and the round-trip fails.
 */
$GLOBALS['__test_wp_salt'] = 'verify-side-different-value';
$bound_out                 = $grabber->_parse_dynamic( $bound_tag );
...
// Tag is modified after signing (simulates a cache-file tamper).
$tampered    = '<!-- mfunc ' . $token . ' call:render_user {"name":"bob"} hmac:' . $valid_hmac . ' --><!-- /mfunc ' . $token . ' -->';
