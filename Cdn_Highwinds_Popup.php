...
class Cdn_Highwinds_Popup {
	...
	/**
	 * Initializes AJAX actions for Highwinds CDN operations.
	 *
	 * @return void
	 */
	public static function w3tc_ajax() {
		...
		add_action( 'w3tc_ajax_cdn_highwinds_authenticate', array( $o, 'w3tc_ajax_cdn_highwinds_authenticate' ) );
		add_action( 'w3tc_ajax_cdn_highwinds_select_host', array( $o, 'w3tc_ajax_cdn_highwinds_select_host' ) );
		add_action( 'w3tc_ajax_cdn_highwinds_configure_host', array( $o, 'w3tc_ajax_cdn_highwinds_configure_host' ) );
		add_action( 'w3tc_ajax_cdn_highwinds_configure_cnames_form', array( $o, 'w3tc_ajax_cdn_highwinds_configure_cnames_form' ) );
		add_action( 'w3tc_ajax_cdn_highwinds_configure_cnames', array( $o, 'w3tc_ajax_cdn_highwinds_configure_cnames' ) );
	}

	/**
	 * Handles the AJAX request to authenticate with Highwinds CDN.
	 *
	 * @return void
	 */
	...
	/**
	 * Handles the AJAX request to select a Highwinds CDN host.
	 *
	 * @return void
	 */
	...
	/**
	 * Handles the AJAX request to configure a Highwinds CDN host.
	 *
	 * @return void
	 *
	 * @throws \Exception If the host scope configuration fails.
	 */
	public function w3tc_ajax_cdn_highwinds_configure_host() {
		...
		$postfix = Util_Admin::custom_message_id(
			array(),
			array(
				'cdn_configuration_saved' => 'CDN credentials are saved successfully',
			)
		);
	...
	/**
	 * Creates a new host on the Highwinds CDN platform.
	 *
	 * @param Cdn_Highwinds_Api $api       The API instance to interact with Highwinds CDN.
	 * @param string            $host_name The name of the new host.
	 *
	 * @return string The hash code of the newly created host.
	 *
	 * @throws \Exception If the host creation or configuration fails.
	 */
	private function _create_host( $api, $host_name ) {
		...
		if ( 0 === $origin_id ) {
			try {
				...
				$origin_response = $api->origin_add(
					array(
						'name'     => $name,
		...
		$configuration['originPullPolicy']         = array(
			array(
	...
	/**
	 * Renders the form to configure CNAMEs for Highwinds CDN.
	 *
	 * @return void
	 */
	...
	/**
	 * Handles the AJAX request to configure CNAMEs for Highwinds CDN.
	 *
	 * @return void
	 */
	public function w3tc_ajax_cdn_highwinds_configure_cnames() {
		...
		try {
			...
			$postfix = Util_Admin::custom_message_id(
				array(),
				array(
					'cdn_cnames_saved' => 'CNAMEs are saved successfully',
				)
			);
			echo 'Location admin.php?page=w3tc_cdn&' . esc_html( $postfix );
			exit();
	...
	/**
	 * Renders the CNAMEs configuration form.
	 *
	 * @param array $details Optional. An array of details to prefill the form.
	 *
	 * @return void
	 */
