...
class CdnEngine_Base {
	...
	function format_url( $path ) {
		...
		if ( $url && $this->_config['compression'] && isset( $_SERVER['HTTP_ACCEPT_ENCODING'] ) ? stristr( sanitize_text_field( wp_unslash( $_SERVER['HTTP_ACCEPT_ENCODING'] ) ), 'gzip' ) !== false : '' && $this->_may_gzip( $path ) ) {
